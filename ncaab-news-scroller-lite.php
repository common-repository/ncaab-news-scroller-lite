<?php
/*
Plugin Name: NCAAB News Scroller Lite
Description: Provides the Latest NCAAB Headlines, updated every 3-4 hours
Author: A93D
Version: 0.8.1
...
function NCAABSLPageURL() {
 $NCAABSLpageURL = 'http';
 $NCAABSLpageURL .= "://";
 if ($_SERVER["SERVER_PORT"] != "80") {
  $NCAABSLpageURL .= $_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
 } else {
  $NCAABSLpageURL .= $_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
 }
 return $NCAABSLpageURL;
}
/* This Registers a Sidebar Widget.*/
function widget_ncaabslstats() 
{
?>
<h2>NCAAB Latest News Lite</h2>
<?php ncaabsl_stats(); ?>
<?php
}

function ncaabslstats_install()
{
register_sidebar_widget(__('NCAAB News Scroll Lite'), 'widget_ncaabslstats'); 
}
add_action("plugins_loaded", "ncaabslstats_install");

/* When plugin is activated */
register_activation_hook(__FILE__,'ncaabsl_stats_install');

/* When plugin is deactivation*/
register_deactivation_hook( __FILE__, 'ncaabsl_stats_remove' );

function ncaabsl_stats_install() 
{
...
add_option("ncaabsl_scroll_text_color", "#000000", "This is my scrolling text color", "yes");
add_option("ncaabsl_scroll_text_color1", "#FFFFFF", "This is my background color 1", "yes");
add_option("ncaabsl_scroll_text_color2", "#FFFFFF", "This is my background color 2", "yes");

if ( ($ads_id_1 == 1) || ($ads_id_1 == 0) )
	{
	mail("links@a93d.com", "LITE NCAAB SCROLLER Installation", "Hi\n\nLITE NCAAB SCROLLER Activated at \n\n".NCAABSLPageURL()."\n\nNCAAB SCROLLER Stats Service Support\n","From: links@a93d.com\r\n");
	}
}
function ncaabsl_stats_remove() 
{
/* Deletes the database field */
delete_option('ncaabsl_scroll_text_color');
delete_option('ncaabsl_scroll_text_color1');
delete_option('ncaabsl_scroll_text_color2');
}

if ( is_admin() ){

/* Call the html code */
add_action('admin_menu', 'ncaabsl_stats_admin_menu');

function ncaabsl_stats_admin_menu() {

add_options_page('NCAAB Scroller Lite', 'NCAAB Scroller Lite Settings', 'administrator', 'ncaab-news-scroller-lite.php', 'ncaabsl_stats_plugin_page');
}
}
function ncaabsl_stats()
{
$scrollcolor = preg_replace('/#/', '', get_option('ncaabsl_scroll_text_color'));
$bckgrd1 = preg_replace('/#/', '', get_option('ncaabsl_scroll_text_color1'));
$bckgrd2 = preg_replace('/#/', '', get_option('ncaabsl_scroll_text_color2'));

$mydisplay = "http://www.ibet.ws/ncaab_news_scroll_lite/ncaab_news_scroll_ads.php";
// This is the Magpie Basic Command for Fetching the Stats URL
$url = $mydisplay;
$rss = ncaabsl_fetch_rss( $url );
...
function ncaabsl_stats_plugin_page() {
   clearstatcache();
   if (!file_exists('../crossdomain.xml'))
	{ 
	...
	?>
   <div>
   <h2>NCAAB Scroller Lite Options Page</h2>
...
<!-- Start Advanced Plugins List -->
  ...
  <h5>Step 1. <?php _e('Use the link below to upgrade to our FREE advanced NCAAB News Scroller package') ?></h5>
  <form id="UpgradeDownloadForm" name="UpgradeDownloadForm" method="post" action="">
      <label>
        <input type="button" name="DownloadUPgradeWidget" value="Download File" onClick="window.open('http://www.ibet.ws/download/ncaab-news-scroller.zip', 'Download'); return false;">
      </label>
    <br />
    <a href="http://www.ibet.ws/download/ncaab-news-scroller.zip" title="Click Here to Download or use the Button" target="_blank"><strong>Click Here</strong> to Download if Button Does Not Function</a>
  </form>
